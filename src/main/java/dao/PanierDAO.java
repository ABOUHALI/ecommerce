...
public class PanierDAO {
	...
	public void addPanier(Panier panier) {
		...
		try {
			...
			ps = conn.prepareStatement(
					"insert into panier (`idclient`, `idproduit`, `prixT`, `quantite`) values(?,?,?,?)");
	...
	public List<Panier> ListPanier() {
		...
		try {
			ps = conn.prepareStatement(
					"select p.*,pr.nom,pr.prix,pr.image ,pr.description,pr.quantite from panier p,produit pr where p.idproduit=pr.idproduit and p.reserve=0");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
	...
	public List<Panier> ListReservation() {
		...
		try {
			ps = conn.prepareStatement(
					"select p.* , c.nom,c.prenom , pr.idproduit,pr.nom ,pr.description,pr.image from \r\n"
			...
			while (rs.next()) {
				...
				p.setDescription(rs.getString("pr.description"));
				Blob blob = rs.getBlob("image");
